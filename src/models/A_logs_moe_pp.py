...
class MoESystem(nn.Module):
    def __init__(self, num_experts, top_k, d_state, input_dim, copies, merge, K, dt_rank):
        ...
        self.shared_expert = A_logs_MoEProjectionLayer(1, 1, d_state, input_dim, copies, merge, K, dt_rank)

    def forward(self, inputs):
        # Pass through the router to get expert weights and indices
        router_output, indices, logits, shared_weight = self.router(inputs)
        ...
        # (B, top_k) * (B, 1)
        combined_output = torch.einsum('bnik,bn->bik', expert_outputs, router_output*(1-shared_weight))

        B = indices.shape[0]
        zeros_tensor = torch.zeros((B, 1), dtype=torch.int64).cuda() # (B, 1)
        shared_output = self.shared_expert(zeros_tensor).cuda()
        combined_output += torch.einsum('bnik,bn->bik', shared_output, shared_weight)

        combined_output = combined_output.mean(dim=0)
...
class NoisyTopkRouter(nn.Module):
    ...
    def __init__(self, n_embed, num_experts, top_k):
        ...
        self.shared_linear = nn.Linear(n_embed, 1)
    ...
    def forward(self, mh_output):
        ...
        # -
        # shared_weight = torch.tanh(self.shared_linear(mh_output))
        # +
        shared_weight = torch.sigmoid(self.shared_linear(mh_output))
        ...
        return router_output.cuda(), indices.cuda(), noisy_logits.cuda(), shared_weight.cuda()  # (B, top_k), (B, top_k)
