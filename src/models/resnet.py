...
from src.models.MamOCL import MambaNeck
...
class Mamba_Model(nn.Module):
    def __init__(self, params, dim_in=512, head='mlp', proj_dim=128, n_classes=100, nf=64):
        super(Mamba_Model, self).__init__()
        self.params = params

        # 初始化模型A
        if self.params.dataset in ['cifar10', 'cifar100', 'tiny']:
            self.modelA = M_ResNet18().to(device)
        elif self.params.dataset in ['imagenet', 'imagenet100']:
            self.modelA = ImageNet_ResNet18().to(device)
        self.head_representation = nn.Sequential(nn.Linear(dim_in, proj_dim))
        self.head_classification = nn.Sequential(nn.Linear(dim_in, n_classes))

        # 初始化模型B
        self.modelB = MambaNeck().to(device)

        
    def forward(self, x, is_simclr=False):
        x = self.modelA(x)
        features = self.modelB(x)
        if is_simclr:
            out = self.head_representation(features)
            return features, out
        else:
    ...
    def logits(self, x):
        # print("x0:", x.shape) # x (20, 3, 32, 32)
        x = self.modelA(x)
        # print("x1:", x.shape) # x (20, 512)
        features = self.modelB(x)
        # features (74, 512)
