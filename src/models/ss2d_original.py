...
def multi_selective_scan(x: torch.Tensor = None,
    ...
    if input_params:
        # print("input!!!")
        dts_input = dts_input.view(B, 4, -1, 16)
        dts_input = dts_input.permute(0, 1, 3, 2)
        Bs_input = Bs_input.view(B, 4, -1, 16)
        Bs_input = Bs_input.permute(0, 1, 3, 2)
        Cs_input = Cs_input.view(B, 4, -1, 16)
        Cs_input = Cs_input.permute(0, 1, 3, 2)
        # print("dts_input", dts_input.shape)
        dts = dts_input
        Bs = Bs_input
        Cs = Cs_input
    ...
    if return_param:
        output = [dts, Bs, Cs] #这里返回的output是参数！不是处理结果！
...
class SS2D(nn.Module):
    ...
    def forward(self, x: torch.Tensor, return_param=False, nrows=None, input_params=False, dts_input=None, Bs_input=None, Cs_input=None):
        xz = self.in_proj(x)
