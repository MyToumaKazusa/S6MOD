...
from src.models.moe_projection import MoEProjectionLayer
# from src.models.A_logs_moe import MoESystem
# from src.models.A_logs_moe_pp import MoESystem
...
def multi_selective_scan(x: torch.Tensor = None,
    ...
    Bs, Cs = torch.split(x_dbl, [N, N], dim=2)

    dts, z_loss = moe_dts_proj(xs, mis_mask)

    if return_param:
        output = [dts, Bs, Cs, z_loss]

    dts = torch.einsum("b k r l, k d r -> b k d l", dts, dt_projs_weight)
    # maybe we can add dimention b in dt_projs_bias
    # dts = moe_dts_proj_weight(dts)
    ...
    # delta_bias = moe_dts_proj_bias.view(-1).to(torch.float)
...
class MultiScan(nn.Module):
    ...
    def scan(self, x, direction='h', mask=None):
        """
    ...
    def reverse(self, x, direction='h', mask=None):
        """
        Input @x: shape [B, D, L]
        Return torch.Tensor: shape [B, D, L]
        """
...
class SS2D(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # ！！！四种投影
        self.x_proj = [
            nn.Linear(d_inner, (self.d_state * 2),
                      bias=False,
        ...
        self.moe_dts_proj = MoEProjectionLayer(num_experts=self.num_experts, input_dim=d_inner, K=self.K, dt_rank=self.dt_rank, d_state=self.d_state)
        ...
        # self.moe_dts_proj_weight, self.moe_dts_proj_bias = MoEProjectionLayer_dts(num_experts=10, topk=2, input_dim=d_inner, K=self.K, dt_rank=self.dt_rank, dt_scale, dt_init, dt_min,
        #                  dt_max, dt_init_floor, **factory_kwargs)
        # 
        ...
        self.A_logs = self.A_log_init(self.d_state,
                                      ...
                                      copies=self.K2,
                                      merge=True)  # (K * D, N)
        # print("A_logs:", self.A_logs.shape)

        # self.A_logs_MoE = MoESystem(num_experts=self.num_experts, top_k=self.top_k, 
        #         d_state=self.d_state, input_dim=d_inner, copies=self.K2, merge=True,
        #         K=self.K, dt_rank=self.dt_rank)
    ...
    def forward_core(self,
        ...
        # 如果当前batch中绝大多数样本都为同一类则可以使用moe，否则使用固定的experts
        # A_logs, load_balance_loss = self.A_logs_MoE(x)
        ...
        if not return_param:
            x = multi_selective_scan(x,
                                     ...
                                     mis_mask=mis_mask,
                                     moe_dts_proj=self.moe_dts_proj)
        else:
    ...
    def forward(self, x: torch.Tensor, return_param=False, mis_mask=None):
        xz = self.in_proj(x)
