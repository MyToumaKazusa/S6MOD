''' Copied from https://github.com/weilllllls/OnPro
'''
import numpy as np
import torch
...
class Buffer(nn.Module):
    def __init__(self, mem_size, img_size, nb_ch, n_classes, drop_method, device):
        super().__init__()
    ...
    def get_all(self):
        all_x = self.bx[:min(self.current_index, self.max_size)]
        all_y = self.by[:min(self.current_index, self.max_size)]
        return all_x, all_y
    ...
    # 蓄水池策略。如果有空就直接添加，如果没空就生成索引决定是否覆盖当前数据
    ...
    def shuffle_(self):
        indices = torch.randperm(self.current_index).to(self.device)
    ...
    def delete_up_to(self, remove_after_this_idx):
        ...
        self.bt = self.bt[:remove_after_this_idx] # 写错？？？
    ...
    # 获取所有数据
    def get_all(self):
        return self.bx[:min(self.current_index, self.max_size)],\
             self.by[:min(self.current_index, self.max_size)]