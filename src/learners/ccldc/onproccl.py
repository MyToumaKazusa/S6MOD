...
class OnProCCLLearner(BaseCCLLearner):
    def __init__(self, args):
        ...
        if self.n_classes_num == 10:
            ...
            self.extra = 1/2
            self.mul_ce_loss = 1
            self.mul_etf_loss = 20 * self.extra
            self.mul_diff_loss = 10 * self.extra
            self.mul_cons_loss = 50 * self.extra
            self.mul_load_loss = 1 * self.extra
            self.num_experts = 10
            ...
            if self.params.mem_size == 10000:
                ...
                self.mul_ce_loss = 1
                self.mul_etf_loss = 10 * self.extra
                self.mul_diff_loss = 3 * self.extra
    ...
    def train_inc(self, dataloader, **kwargs):
        ...
        if task_id == 0:
            ...
            for batch_idx, batch in enumerate(dataloader):
                ...
                with torch.autocast(device_type='cuda', dtype=torch.float16):
                    ...
                    #ce1
                    y_pred_linear1 = self.model1.logits(self.transform(x))
                    ce_loss1 = F.cross_entropy(y_pred_linear1, y)
                    wandb.log({"ce_loss": ce_loss1.item()})
                    loss1 += ce_loss1

                    #etf_loss#############################
                    features1, outputs1 = self.model1(self.transform_train(x), labels=y, is_outputs=True)
                    features1 = self.pre_logits(self.etf_transform(features1))
                    etf_loss1 = self.etf_criterion(features1, self.etf_classifier[:, y].t())
                    ...
                    # #load_balancing_loss#############################
                    load_balancing_loss1 = outputs1['load_balancing_loss']
                    load_balancing_loss1 *= self.mul_load_loss
                    loss1 += load_balancing_loss1
                    ...
                    if self.use_cons_loss:
                        cons_loss1 = self.calculate_cons_loss(outputs1, y)
                    ...
                    y_pred_linear2 = self.model2.logits(self.transform(x))
                    ce_loss2 = F.cross_entropy(y_pred_linear2, y)
                    loss2 += ce_loss2

                    #etf_loss2#############################
                    features2, outputs2 = self.model2(self.transform_train(x), labels=y, is_outputs=True)
                    features2 = self.pre_logits(self.etf_transform(features2))
                    etf_loss2 = self.etf_criterion(features2, self.etf_classifier[:, y].t())
                    etf_loss2 = etf_loss2.mean() #或许可以测试一个不mean的结果
                    etf_loss2 *= self.mul_etf_loss
                    loss2 += etf_loss2

                    load_balancing_loss2 = 0
                    # #load_balancing_loss2#############################
                    ...
                    #cons_loss2#############################
                    if self.use_cons_loss:
                        cons_loss2 = self.calculate_cons_loss(outputs2, y)
                ...
                if num_d % self.print_num == 0 or batch_idx == 1:
                    print('==>>> it: {}, loss: ce {:.2f} + ins {:.4f} + ETF {:.4f} + cons {:.4f} + diff {:.4f} + load {:.4f}= {:.6f}, {}%'
                        .format(batch_idx, ce_loss1, ins_loss1, etf_loss1, cons_loss1, diff_loss1, load_balancing_loss1, loss1, 100 * (num_d / self.total_samples)),flush=True)

                if num_d % self.print_num == 0 or batch_idx == 1:
                    print('==>>> it: {}, loss: ce {:.2f} + ins {:.4f} + ETF {:.4f} + cons {:.4f} + diff {:.4f} + load {:.4f}= {:.6f}, {}%'
                        .format(batch_idx, ce_loss2, ins_loss2, etf_loss2, cons_loss2, diff_loss2, load_balancing_loss2, loss2, 100 * (num_d / self.total_samples)),flush=True)
                    self.myprint()
            ...
            for batch_idx, batch in enumerate(dataloader):
                ...
                with torch.autocast(device_type='cuda', dtype=torch.float16):
                    ...
                    # ce_loss
                    y_pred_linear1 = self.model1.logits(self.transform(mem_x))
                    if batch_idx != 0:
                        ce_loss1 = self.loss_mixup(y_pred_linear1, mem_y_mix)
                    else:
                    ...
                    if batch_idx != 0:
                        ...
                        input = self.model1(self.transform(mem_x), labels=mem_y_mix[:,0])
                        input = self.pre_logits(self.etf_transform(input))
                        etf_loss1 = self.etfloss_mixup(input, mem_y_mix)
                    else:
                        mem_y = mem_y.long()
                        input = self.model1(self.transform(mem_x), labels=mem_y)
                        input = self.pre_logits(self.etf_transform(input))
                        etf_loss1 = self.etf_criterion(input, self.etf_classifier[:, mem_y].t())
                        etf_loss1 = etf_loss1.mean()
                    etf_loss1 *= self.mul_etf_loss
                    loss1 += etf_loss1
                    ########################################################################
                    # # diff_loss   
                    y_pred_etf1 = self.etf_predict(self.model1(self.transform(mem_x)), self.etf_classifier)                    
                    diff_loss1 = F.kl_div(F.log_softmax(y_pred_linear1, dim=1), y_pred_etf1, reduction='batchmean')
                    loss1 += diff_loss1
                    ########################################################################
                    # cons_loss
                    cons_loss1 = 0
                    load_balancing_loss1_1 = 0
                    load_balancing_loss1_2 = 0
                    if self.use_cons_loss:
                        _, outputs1 = self.model1(self.transform(x), labels=y, is_outputs=True)
                        cons_loss1 = self.calculate_cons_loss(outputs1, y)
                        cons_loss1 *= self.mul_cons_loss
                        if batch_idx != 0:
                    ...
                    # ce_loss
                    y_pred_linear2 = self.model2.logits(self.transform(mem_x))
                    if batch_idx != 0:
                        ce_loss2 = self.loss_mixup(y_pred_linear2, mem_y_mix)
                    else:
                    ...
                    if batch_idx != 0:
                        mem_y_mix = mem_y_mix.long()
                        input = self.model2(self.transform(mem_x), labels=mem_y_mix[:,0])
                        input = self.pre_logits(self.etf_transform(input))
                        etf_loss2 = self.etfloss_mixup(input, mem_y_mix)
                    else:
                        mem_y = mem_y.long()
                        input = self.model2(self.transform(mem_x), labels=mem_y)
                        input = self.pre_logits(self.etf_transform(input))
                        etf_loss2 = self.etf_criterion(input, self.etf_classifier[:, mem_y].t())
                        etf_loss2 = etf_loss2.mean()
                    etf_loss2 *= self.mul_etf_loss
                    loss2 += etf_loss2
                    ########################################################################
                    # # diff_loss   
                    y_pred_etf2 = self.etf_predict(self.model2(self.transform(mem_x)), self.etf_classifier)                    
                    diff_loss2 = F.kl_div(F.log_softmax(y_pred_linear2, dim=1), y_pred_etf2, reduction='batchmean')
                    loss2 += diff_loss2
                    ########################################################################
                    # cons_loss
                    ...
                    if self.use_cons_loss:
                        _, outputs2 = self.model2(self.transform(x), labels=y, is_outputs=True)
                        cons_loss2 = self.calculate_cons_loss(outputs2, y)
                        cons_loss2 *= self.mul_cons_loss
                        if batch_idx != 0:
                            labels = mem_y_mix[:,0].long()
                            ...
                            lables = mem_y.long()
                        _, mem_outputs2 = self.model2(self.transform_train(mem_x), labels=lables, is_outputs=True)
                        loss2 += cons_loss2
                ...
                if num_d % self.print_num == 0 or batch_idx == 1:
                    print('==>>> it: {}, loss: ce {:.2f} + ins {:.4f} + ETF {:.4f} + cons {:.4f} + diff {:.4f} + load {:.4f}= {:.6f}, {}%'
                        .format(batch_idx, ce_loss1, ins_loss1, etf_loss1, cons_loss1, diff_loss1, load_balancing_loss1_1, loss1, 100 * (num_d / self.total_samples)),flush=True)

                if num_d % self.print_num == 0 or batch_idx == 1:
                    print('==>>> it: {}, loss: ce {:.2f} + ins {:.4f} + ETF {:.4f} + cons {:.4f} + diff {:.4f} + load {:.4f}= {:.6f}, {}%'
                        .format(batch_idx, ce_loss2, ins_loss2, etf_loss2, cons_loss2, diff_loss2, load_balancing_loss2_1, loss2, 100 * (num_d / self.total_samples)),flush=True)
                    self.myprint()
        ...
        if task_id != -1:
            ...
            x = x[:999].cuda()
            y = y[:999].cuda()
            x = self.model1.after_backbone(self.transform(x))
            # print("x", x.shape)
            self.plot_embedding_2d(x, y, save_path=f"embedding_plot_onproccldc_c100.pdf")
