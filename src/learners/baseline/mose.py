...
from src.utils.loss import sup_con_loss
from src.utils import get_transform
from src.utils.rotation_transform import RandomFlip

from src.learners.baseline.base import BaseLearner
from src.utils.losses import SupConLoss
from src.buffers.buffermose import Buffer
from src.utils import name_match
from src.models.Resnet18_SD import ResNetSD
...
class MOSE(BaseLearner):
    def __init__(self, args):
        ...
        self.use_ncm = True
        input_size = 512
        self.buffer = Buffer(self.params, input_size).cuda()
        self.buffer_per_class = 7
        self.buffer_batch_size = self.params.buffer_batch_size
        self.buffer_cur_task = (self.buffer_batch_size // 2) - self.params.batch_size

        if self.params.dataset == "cifar10":
            self.total_samples = 10000
        elif "cifar100" in self.params.dataset:
            self.total_samples = 5000
        elif self.params.dataset == "tiny_imagenet":
            self.total_samples = 1000
        self.print_num = self.total_samples // 10

        self.transform = get_transform('ocm', input_size)
    ...
    def load_model(self, **kwargs):
        if self.params.dataset == 'cifar10' or self.params.dataset == 'cifar100' or self.params.dataset == 'tiny':
            return ResNetSD(nclasses=self.params.n_classes).to(device)

    def load_criterion(self):
        return SupConLoss(self.params.temperature) 

    def train_any_task(self, train_loader, **kwargs):
        epoch = 0
        ...
        for batch_idx, (x, y) in enumerate(train_loader):
            ...
            if num_d % self.print_num == 0 or batch_idx == 1:
                print('==>>> it: {}, loss: ce {:.2f} + ins {:.4f} + dist {:.4f}= {:.6f}, {}%'
                    .format(batch_idx, ce_loss, ins_loss, distill_loss, loss, 100 * (num_d / self.total_samples)),flush=True)
                self.myprint()

    def train(self, dataloader, **kwargs):
        if self.params.training_type == "inc":
            self.train_any_task(dataloader, **kwargs)
